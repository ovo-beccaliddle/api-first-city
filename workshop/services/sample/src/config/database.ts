...
export const initializeDatabase = async (): Promise<DataSource> => {
  try {
    logger.info(
      `Attempting to connect to database at ${config.db.host}:${config.db.port}/${config.db.database}`
    );
    logger.info(
      `Database connection details: Host=${config.db.host}, Port=${config.db.port}, User=${config.db.username}, Database=${config.db.database}`
    );
    ...
    logger.error(
      'Please ensure PostgreSQL is running and the connection details are correct in .env file'
    );
